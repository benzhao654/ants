...
class Ant(Insect):
    ...
    def buff(self):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if not self.buffed and self.damage:
            self.damage *= 2
            self.buffed = True
        # END Problem EC
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cnt = 0
        temp = self.place
        while choose_bee(temp.bees) == None or self.min_range > cnt or cnt > self.max_range:
            temp = temp.entrance
            if temp.is_hive:
                return None
            cnt += 1
        #print(f'{cnt} {self.min_range} {self.max_range}')
        return choose_bee(temp.bees)  # REPLACE THIS LINE
        # END Problem 3 and 4
    ...
    def action(self, gamestate):
        if self.chew_countdown == 0:
            if self.place.bees:
                ...
                tempbee = choose_bee(self.place.bees)
                tempbee.reduce_health(tempbee.health)
        else:
        ...
        if QueenAnt.number > 1:
            ...
            while temp != None:
                if temp.ant is not None:
                    temp.ant.buff()
                    if temp.ant.is_container:
                        if temp.ant.ant_contained:
                            temp.ant.ant_contained.buff()
                temp = temp.exit
